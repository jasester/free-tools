    ...
    <div className="flex min-h-screen flex-col">
      ...
      <main className="mx-auto w-full max-w-6xl flex-1 px-4 py-8">
        <div className="animate-fade-in-up">
          <h1 className="mb-2 text-3xl font-bold font-display text-light-900 dark:text-dark-50">JS/HTML Minifier & Formatter</h1>
          <p className="mb-6 text-light-500 dark:text-dark-200">Minify or format JavaScript and HTML code instantly.</p>
        </div>
        ...
        <div className="mt-6 flex flex-wrap gap-2 animate-fade-in-up">
          <div className="flex gap-1 rounded-lg bg-gray-100 dark:bg-dark-600/50 p-1">
            <button onClick={() => setMode('js')} className={`rounded-md px-4 py-1.5 text-sm font-medium transition-all ${mode === 'js' ? 'btn-gradient text-white' : 'text-light-500 dark:text-dark-200 hover:text-light-900 dark:hover:text-dark-50'}`}>JavaScript</button>
            <button onClick={() => setMode('html')} className={`rounded-md px-4 py-1.5 text-sm font-medium transition-all ${mode === 'html' ? 'btn-gradient text-white' : 'text-light-500 dark:text-dark-200 hover:text-light-900 dark:hover:text-dark-50'}`}>HTML</button>
          </div>
          <div className="flex gap-1 rounded-lg bg-gray-100 dark:bg-dark-600/50 p-1">
            <button onClick={() => setAction('minify')} className={`rounded-md px-4 py-1.5 text-sm font-medium transition-all ${action === 'minify' ? 'btn-gradient text-white' : 'text-light-500 dark:text-dark-200 hover:text-light-900 dark:hover:text-dark-50'}`}>Minify</button>
            <button onClick={() => setAction('format')} className={`rounded-md px-4 py-1.5 text-sm font-medium transition-all ${action === 'format' ? 'btn-gradient text-white' : 'text-light-500 dark:text-dark-200 hover:text-light-900 dark:hover:text-dark-50'}`}>Format</button>
          </div>
        </div>
        ...
        <div className="mt-6 grid gap-4 lg:grid-cols-2 animate-fade-in-up">
          <div className="glass-card rounded-xl p-4">
            <div className="mb-2 flex items-center justify-between">
              <label className="text-sm font-medium text-light-700 dark:text-dark-100">Input ({mode === 'js' ? 'JavaScript' : 'HTML'})</label>
            ...
            <textarea value={input} onChange={(e) => setInput(e.target.value)} className="input-dark h-64 w-full resize-none rounded-lg p-3 font-mono text-sm text-light-900 dark:text-dark-50" placeholder={`Paste your ${mode === 'js' ? 'JavaScript' : 'HTML'} code here...`} />
          </div>
          <div className="glass-card rounded-xl p-4">
            <div className="mb-2 flex items-center justify-between">
        ...
        <section className="glass-card mt-10 rounded-xl p-6 animate-fade-in-up">
          <h2 className="mb-2 text-lg font-semibold font-display text-light-900 dark:text-dark-50">About JS/HTML Minifier</h2>
          <p className="text-sm leading-relaxed text-light-500 dark:text-dark-200">Minify JavaScript and HTML code to reduce file size for faster page loading. Format minified code for better readability.</p>
        </section>
      </main>
      <Footer />
    </div>
