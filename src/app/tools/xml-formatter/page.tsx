    ...
    <div className="flex min-h-screen flex-col">
      ...
      <main className="mx-auto w-full max-w-6xl flex-1 px-4 py-8">
        <div className="animate-fade-in-up">
          <h1 className="mb-2 text-3xl font-bold font-display text-dark-50">XML Formatter & Sorter</h1>
          <p className="mb-6 text-dark-200">Format, minify and sort XML nodes instantly.</p>
        </div>
        ...
        <div className="mt-6 grid gap-4 lg:grid-cols-2 animate-fade-in-up">
          <div className="glass-card rounded-xl p-4">
            <div className="mb-2 flex items-center justify-between">
              <label className="text-sm font-medium text-dark-100">Input XML</label>
              <button onClick={() => setInput('')} className="text-xs text-dark-200 hover:text-dark-100">Clear</button>
            </div>
            <textarea value={input} onChange={(e) => setInput(e.target.value)} className="input-dark h-64 w-full resize-none rounded-lg p-3 font-mono text-sm text-dark-50" placeholder="Paste your XML here..." />
          </div>
          <div className="glass-card rounded-xl p-4">
            <div className="mb-2 flex items-center justify-between">
        ...
        <section className="glass-card mt-10 rounded-xl p-6 animate-fade-in-up">
          <h2 className="mb-2 text-lg font-semibold font-display text-dark-50">About XML Formatter</h2>
          <p className="text-sm leading-relaxed text-dark-200">Format, minify and sort XML nodes alphabetically. Useful for comparing XML files and cleaning up messy XML data.</p>
        </section>
      </main>
      <Footer />
    </div>
